...
A process also reports launch `activeConfig`, `appliedConfigVersion`, desired/loaded versions, local overrides, PID, actual process start time, optional miner version, shares, and pool observation. Process uptime is elapsed time since the reported launch, not measured mining availability. Unknown launch times stay unknown. Stale or disconnected process uptime is null instead of continuing to imply confirmed runtime. Client display version and boot ID are sent during registration. Errors from stopped, explicitly disabled processes remain in process diagnostics but do not override fleet status/reason or add an error count. Enabled-process errors and errors from a still-running disabled process remain operational errors.
...
- `GET /monitoring/rules`, `PUT /monitoring/rules`: shared rules. Defaults: offline/stale/zero/temperature/missing-GPU/reject/crash-loop checks enabled; temperature 85°C, start grace 120 seconds, reject threshold 5% after at least 20 shares, three unexpected exits in 15 minutes. The stale rule also flags a running, unpaused process with missing/expired hashrate after the same startup grace (`zeroGraceSeconds`). It remains distinct from a measured zero and identifies the process whose logs need checking.

The `miner_errors` rule (enabled by default) warns after at least three error-level log entries within five minutes from a currently running, unpaused process. Both receipt and original observation times must be recent, and the observation must be after that process's launch. Each query is bounded to the newest 200 matching entries and five seconds. Older backlog and previous launches are excluded. The warning respects maintenance and resolves after its window expires or the process stops; it never triggers automatic recovery. The admin rule switch and `PUT /monitoring/rules` control it, and OpenAPI describes all rule fields and bounds.
...
## September 15 regression repair / desktop 1.4.4

The Fleet page restores direct **Play / Pause** for each rig, selected rigs and the whole fleet. **Play all / Pause all** use all active inventory across pages and filters; offline/unsupported rigs are skipped with visible counts. Play respects disabled processes and chooses CPU scope for CPU-only rigs. Pause uses ALL, including pending starts. Existing `POST /v1/commands` and its access boundaries/idempotency/receipts are unchanged; the UI dispatches eight targets per request and tracks their individual results. A request acknowledgment is never presented as mining success.

`GET /v1/rigs?sort=activity&order=asc` sorts mining first, other connected rigs next, then overdue and offline rigs, with name/ID tie-breaks and matching cursors. The API default remains `name`; the UI explicitly requests `activity`. Last contact and telemetry freshness remain distinct. The UI uses historical `lastSeen` when new telemetry is absent and labels legacy versions that were never sent. Offline processes display Offline rather than claiming to be stopped. Group/tag/attention filters and saved views live under **More filters & saved views**. The September 18 UX repair supersedes this release's collapsed chart/hardware layout; see below.

New `agent-disconnected` event details include transport `code` and `reason` when available. Client 1.4.4 restores Nanominer file telemetry and retries an unacknowledged registration; registration and reporting still require no API key. See the [regression findings and validation limits](audits/regression-repair-2026-09-15/README.md).

## September 18 admin UX repair

The Fleet page puts **total GPU/CPU hashrate and history above the rig list**, visible without expansion. Totals use `fleet/summary`; charts use `metrics/hashrate` and refresh every minute. CPU/GPU and different algorithms stay separate. Chart tabs choose scope/algorithm and the period selector supports 1h, 24h, 7d, 30d and 90d. Current totals and history follow the same fleet filters across every page; the heading explicitly identifies a filtered view. Missing samples remain gaps, and the chart tooltip exposes sample coverage. Secondary diagnostics remain in Monitoring and rig details.

Every rig row/card shows prominent process rates, engine/algorithm, hardware models, CPU/GPU temperature and load, GPU watts and RAM usage. Unavailable or stale sensor readings remain blank dashes; sensors are matched by stable device ID. Long startup errors become compact issue links with full diagnostics in rig details. The mining-first default sort, group/tag filters, saved views, selection controls and advanced commands remain available.

Each rig has **one primary control**: Pause while any process is owned or recovering, Play when stopped, Cancel start during preparation, and disabled Offline when disconnected. A pending Pause prevents duplicate clicks. Play refreshes target state and starts only enabled, stopped scopes; it does not restart an already running engine. Fleet Play/Pause still targets active inventory across every page and filter; selected actions use explicit selected IDs. Newer Pause still cancels unsent Play batches and targets ALL to cancel preparation/recovery. No command protocol or API permission boundary changed.

Action toasts show progress, then terminal success/failure from command receipts. Successful notifications dismiss after five seconds; failures stay longer and remain available in row/command details. Partial bulk failures are reported as failures, not blanket success. Saving rig settings, monitoring rules and configurations, creating/revoking API keys, and advanced commands also produce feedback; dispatch-only configuration rollouts explicitly say results are still pending. See the [comparison, verification and deployment notes](audits/admin-ux-2026-09-18.md).
